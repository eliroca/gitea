...
func TestPullDiff_Empty(t *testing.T) {
	onGiteaRun(t, func(t *testing.T, giteaURL *url.URL) {
		session := loginUser(t, "user1")
		...
		// Create an empty branch (no new commits, same as master)
		_, _, err := gitcmd.NewCommand("branch", "empty-branch").
			WithDir(path).
			RunStdString(t.Context())
		assert.NoError(t, err)

		// Create PR with no commits (both branches at same commit)
		req = NewRequestWithJSON(t, http.MethodPost, fmt.Sprintf("/api/v1/repos/%s/%s/pulls", "user1", repoName), &api.CreatePullRequestOption{
			Head:  "empty-branch",
			Base:  "master",
			Title: "Empty PR",
		...
		doc := NewHTMLParser(t, resp.Body)
		// We expect to see 0 files because there are no changes between the branches
